...
def support_matrix(package_version: str, revision: str, created: str) -> dict:
    return {
        "architectures": [
            {
                "architecture": "x86_64",
                "note": "Current reproducible x86 proofs run under QEMU/TCG and OVMF; no physical-hardware proof is claimed.",
                "proof_commands": ["make abi", "make test", "make qemu", "make qemu-uefi", "make qemu-uefi-loader", "make qemu-uefi-tcg2"],
                "status": "live",
                "support_scope": "emulator-firmware-machine-proof",
            },
            {
                "architecture": "aarch64",
                "note": "When included in the source revision, the QEMU virt machine proof exercises the little-endian v1.1 ABI, X0 handoff, bounded parser, and a CRC-verified QEMU-generated Device Tree; no AArch64 OS port is claimed.",
                "proof_commands": ["make qemu-aarch64"],
                "status": "live",
                "support_scope": "machine-proof",
            },
            {
                "architecture": "riscv64",
                "note": "When included in the source revision, the OpenSBI/QEMU virt proof exercises the little-endian v1.1 ABI, A0 handoff, bounded parser, and a CRC-verified QEMU-generated Device Tree; no RISC-V OS port is claimed.",
                "proof_commands": ["make qemu-riscv64"],
                "status": "live",
                "support_scope": "machine-proof",
            },
            {
        ...
        "ports": [
            {
                ...
                "note": "The hosted adapter gate is reproducible at the release source revision. The checked serial artifact is a historical full TinaLinux boot under QEMU/KVM, not physical-hardware evidence.",
                "port": "tinalinux",
                "proof_commands": [
                    "make -C ports/tinalinux scaffold-check CROSS=",
                    "make -C ports/tinalinux test",
                ],
                "proofs": [
                    {
                        "core_revision": revision,
                        "proof_command": "make -C ports/tinalinux test",
                        "proof_scope": "hosted-adapter",
                        "replay": "reproducible-from-source",
                        "substrate": "host-process",
                    },
                    {
                        "core_revision": "historical BBP v1.1 snapshot; exact commit not recorded",
                        "path": "ports/tinalinux/test/serial.log",
                        "proof_scope": "full-os-boot",
                        "replay": "recorded-only; external TinaLinux tree not archived here",
                        "substrate": "qemu-kvm-emulator",
                    },
                ],
                "status": "host-tested",
            },
            {
                "evidence": "ports/minix/test/serial-all6-consumers.log",
                "note": "serial.log is a Limine adapter machine harness, while serial-all6-consumers.log is the historical full MINIX OS boot. Current hosted replay does not reproduce either archived machine run.",
                "port": "minix",
                "proof_commands": [
                    "make -C ports/minix scaffold-check CROSS=",
                    "make -C ports/minix test-hosted",
                ],
                "proofs": [
                    {
                        "core_revision": revision,
                        "proof_command": "make -C ports/minix test-hosted",
                        "proof_scope": "hosted-adapter",
                        "replay": "reproducible-from-source",
                        "substrate": "host-process-synthetic-snapshot",
                    },
                    {
                        "core_revision": "b2ea55e",
                        "path": "ports/minix/test/serial.log",
                        "proof_scope": "adapter-machine-harness",
                        "replay": "recorded-only; machine harness is not replayed by the hosted gate",
                        "substrate": "limine-machine-harness",
                    },
                    {
                        "core_revision": "b2ea55e",
                        "path": "ports/minix/test/serial-all6-consumers.log",
                        "proof_scope": "full-os-boot",
                        "replay": "recorded-only; external MINIX tree not archived here",
                        "substrate": "qemu-limine-minix",
                    },
                ],
                "status": "host-tested",
            },
            {
                ...
                "note": "The checked serial artifact is hosted. An in-kernel QEMU boot is reported in CONFORMANCE.md, but no raw in-kernel serial artifact is archived, so it is not release evidence.",
                "port": "linux01",
                "proof_commands": [
                    "make -C ports/linux01 scaffold-check CROSS=",
                    "make -C ports/linux01 test",
                ],
                "proofs": [
                    {
                        "core_revision": revision,
                        "proof_command": "make -C ports/linux01 test",
                        "proof_scope": "hosted-adapter",
                        "replay": "reproducible-from-source",
                        "substrate": "host-process-fixed-ram-model",
                    },
                    {
                        "core_revision": "9257abe report provenance",
                        "path": "ports/linux01/test/serial.log",
                        "proof_scope": "hosted-adapter",
                        "replay": "recorded hosted output; current command is separately replayable",
                        "substrate": "host-process-fixed-ram-model",
                    },
                    {
                        "core_revision": "9257abe (reported)",
                        "proof_scope": "reported-full-os-boot",
                        "replay": "unarchived-not-replayable",
                        "substrate": "qemu-linux-0.01",
                    },
                ],
                "status": "host-tested",
            },
            {
                ...
                "note": "run.log is the hosted six-tag proof. serial.log is a historical external Josh OS record with five tags; the external OS boot is not replayed here.",
                "port": "josh",
                "proof_commands": [
                    "make -C ports/josh scaffold-check CROSS=",
                    "make -C ports/josh test",
                ],
                "proofs": [
                    {
                        "core_revision": revision,
                        "proof_command": "make -C ports/josh test",
                        "proof_scope": "hosted-adapter",
                        "replay": "reproducible-from-source",
                        "substrate": "host-process-synthetic-limine-data",
                    },
                    {
                        "core_revision": "ae5d3e2 report provenance",
                        "path": "ports/josh/test/run.log",
                        "proof_scope": "hosted-adapter",
                        "replay": "recorded hosted output; current command is separately replayable",
                        "substrate": "host-process-synthetic-limine-data",
                    },
                    {
                        "core_revision": "ae5d3e2",
                        "path": "ports/josh/test/serial.log",
                        "proof_scope": "full-os-boot",
                        "replay": "recorded-only; external Josh tree not archived here",
                        "substrate": "qemu-limine-josh",
                    },
                ],
                "status": "host-tested",
            },
        ...
        "source_revision_scope": "Identifies this metadata and the supplied release artifacts. Historical port records retain their own core provenance and do not prove an external OS boot at this revision.",
