...
| BBP v2 contiguous capsule Draft | 🟢 HOST-TESTED / MULTI-ISA COMPILED | offline-only parser, failure-atomic deterministic builder, canonical digest, v1.1 bridge, fuzzing, native Profile 0, and C/Rust/host package surfaces; Waves 26-28 close control-alias/stale-view gaps, add a generated 46-case C/Rust/Python repository corpus, and enforce explicit inline-versus-external bridge ownership, while profile identity, critical entries, registries, auth, deployment, and freeze remain unresolved |
...
| Experimental P-256 public-key policy | 🟢 HOST-TESTED / MULTI-ISA COMPILED | `make auth2-test`, `make auth2-freestanding-test`, `make auth2-sanitize-test`, and `make auth2-portability` prove OpenSSL interoperability plus a zero-allocation pinned-BearSSL verifier for exact extents, root-signed manifests, low-S policy, key lifecycle, release/recovery roles, failure atomicity, closed symbols, and x86_64/AArch64/RV64 compilation; borrowed inputs must remain caller-owned and immutable, and this is not firmware integration, monotonic storage, Secure Boot, or deployment |
...
| Experimental TPM2 NV floor provider | 🟢 HOST-TESTED | `make tpm2-nv-response-test` proves exact index-authorized command/response codecs, metadata and Name checks, UID-pinned private UNIX transport with a total deadline, finite trusted provider locking, deterministic two-thread serialization through independently opened lock descriptors, ambiguous increment recovery, regression detection, and A/B journal composition against a fake authority; no cross-process race proof, live `swtpm`, provisioning, daemon-state freshness, physical TPM, or firmware proof |
